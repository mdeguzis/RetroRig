...
void ReScan(int i)
{
  fprintf(stdout,"debug jc: ReScan\n");
  #ifdef HAS_SDL_JOYSTICK
...
void CJoystick::Initialize()
{
  ...
  fprintf(stdout,"debug jc: CJoystick::Initialize\n");
  
  // jc debug: register interrupt handler for SIGUSR2 to have XBMC re-initialze the gamepads
  ...
  fprintf(stdout,"debug jc: int m_Amount[MAX_AXES]    : %d\n",m_Amount[0]);
  fprintf(stdout,"debug jc: int m_AxisId              : %d\n",m_AxisId);
  fprintf(stdout,"debug jc: int m_ButtonId            : %d\n",m_ButtonId);
  fprintf(stdout,"debug jc: uint8_t m_HatState        : %d\n",m_HatState);
  fprintf(stdout,"debug jc: int m_HatId               : %d\n",m_HatId);
  fprintf(stdout,"debug jc: int m_JoyId               : %d\n",m_JoyId);
  fprintf(stdout,"debug jc: int m_NumAxes             : %d\n",m_NumAxes);
  fprintf(stdout,"debug jc: int m_DeadzoneRange       : %d\n",m_DeadzoneRange);
  
  if (m_joystickEnabled) 
  {
    fprintf(stdout,"debug jc: bool m_joystickEnabled    : true\n"); 
  } 
  else
  {
    fprintf(stdout,"debug jc: bool m_joystickEnabled    : false\n");
  }
...
{
  fprintf(stdout, "debug jc: CJoystick::SetEnabled\n");
